...
public class UsersService implements IUsersService {
    ...
    public Map<String, Object> checkUserStatus(String userJwt) {
        ...
        if (!status.equals(UsersStatus.NORMAL)) {
            if (status.equals(UsersStatus.SLEEPING)) {
                ...
                return ajaxResponseMgr.makeResponse("10004"); // 휴면중인 회원입니다.
            }
            else if (status.equals(UsersStatus.BLACKLIST)) {
                logger.info("User found but, status is '" + status.getValue() + "'. (email:" + email + ")");
                return ajaxResponseMgr.makeResponse("10005"); // 블랙리스트 회원입니다.
            }
            else if (status.equals(UsersStatus.DEREGISTERED)) {
                logger.info("User found but, status is '" + status.getValue() + "'. (email:" + email + ")");
                return ajaxResponseMgr.makeResponse("10006"); // 탈퇴한 회원입니다.
            }
            else {
                logger.info("User found but, status is 'Undefined'. (email:" + email + ")");
                return ajaxResponseMgr.makeResponse("10003"); // 회원 상태값이 미정의된 값입니다.
            }
        }
        ...
        if (accessibleTime != null && System.currentTimeMillis() < accessibleTime) {
            ...
            return ajaxResponseMgr.makeResponse("10002"); // 아직 사용할 수 없는 계정입니다.
        }
    ...
    // 회원 추가
    public Map<String, Object> insertUser(String email, String name) {
        if (email == null || name == null) {
            logger.error("'email' or 'name' is null! (email:" + email + ", name:" + name + ")");
            return ajaxResponseMgr.makeResponse("00101"); // 필수 인자값이 비었습니다.
        }
        ...
        if (StringUtil.isEmail(email) == false) {
            ...
            return ajaxResponseMgr.makeResponse("00103"); // 올바른 이메일 형식이 아닙니다.
        }

        if (name.length() == 0) {
            logger.error("'name's length is zero!");
            return ajaxResponseMgr.makeResponse("00105"); // 이름이 너무 짧습니다.
        }
        ...
        if (selectedUser != null) {
            ...
            return ajaxResponseMgr.makeResponse("00104"); // 사용중인 이메일입니다.
        }
        ...
        try { // JPA - Insert
            ...
            return ajaxResponseMgr.makeResponse("10102"); // 회원 DB추가중 오류가 발생했습니다.
        }
